...
import { BehaviorSubject, combineLatest, firstValueFrom, Observable, of, scan, switchMap, takeWhile, tap, timer } from 'rxjs';
import { catchError, concatMap, first, map, retry, take } from 'rxjs/operators';
...
export class LiveResultService {
  ...
  #replayRadio(_race: IRace, drivers: IDriver[]): Observable<TeamRadio[] | null> {
    return this.#replayState$.pipe(
      truthy(),
  ...
  #replayPitStops(_race: IRace, drivers: IDriver[], teams: ITeam[]) {
    return this.#replayState$.pipe(
      truthy(),
  ...
  #replayIntervals(_race: IRace, drivers: IDriver[]): Observable<IDriverInterval[]> {
    return this.#replayState$.pipe(
      truthy(),
  ...
  #replayRaceControl(_race: IRace, drivers: IDriver[]): Observable<RaceControl[]> {
    return this.#replayState$.pipe(
      truthy(),
  ...
  #replayStints(_race: IRace, drivers: IDriver[]) {
    return this.#replayState$.pipe(
      truthy(),
      map(state => {
