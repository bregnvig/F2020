...
const openF1Map = ({ laps, drivers }: OpenF1SectorParams): IDriverSector[] => {
  ...
  const getSectorColor = (duration: number | undefined, personalBest: number | undefined, best: number): SectorColor | undefined => {
    if (!duration) return;
    if (duration === best) return 'purple';
    if (personalBest && duration === personalBest) return 'green';
    return 'yellow';
  };
